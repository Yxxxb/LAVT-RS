...
# import data.transforms_video as T
...
class ModulatedDetection(torchvision.datasets.CocoDetection):
    def __init__(self, args, img_folder, ann_file, num_frames, transforms, return_masks):
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:
            ...
            target['caption'] = sentences
            target['caption_mask'] = attentions
            ...
            for mask in target['masks']:
                transform = torchT.ToPILImage()
                mask = transform(mask).convert('L')
                masks.append(mask)

            if self._transforms is not None:
                img, masks = self._transforms(seq_images, masks)
        ...
        target['frames_idx']=torch.zeros(self.num_frames, dtype=torch.long)
        target["valid"] = torch.ones(self.num_frames, dtype=torch.long)
        target.pop('boxes')
        target.pop('labels')
        target.pop('image_id')
        target.pop('area')
...
def build(dataset_file, image_set, args):
    root = Path("./refer/data/")
    assert root.exists(), f"provided COCO path {root} does not exist"
    mode = "instances"
    dataset = dataset_file
    PATHS = {
        "train": (root / "images/mscoco/images/train2014", root / dataset / f"{mode}_{dataset}_train.json"),
        "val": (root / "images/mscoco/images/train2014", root / dataset / f"{mode}_{dataset}_val.json"),
    }
    ...
    dataset = ModulatedDetection(
        ...
        transforms=SimpleTransforms(args),
